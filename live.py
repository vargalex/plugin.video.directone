    ...
    for program in epg:
        ...
        if show_item:
            if last_program is not None:
                ...
                if (
                    last_start
                    + datetime.timedelta(minutes=last_program["duration"] + EPG_GAP)
                    > now
                ):
...
def channels(sl):
    ...
    epg = utils.call(
        sl,
        lambda: sl.epg(channelsData, today, today + datetime.timedelta(days=1), False),
    )
    ...
    if channelsData:
        for channel in channelsData:
            ...
            plot = (
                generate_plot(
                    [x for x in epg if stationid in x][0][stationid],
                    channel["title"],
                    4,
                )
                if epg
...
def play(sl, lid, stationid, askpin):
    ...
    epg = utils.call(
        sl,
        lambda: sl.epg(
            [{"stationid": stationid}], today, today + datetime.timedelta(days=1), False
        ),
    )
